...
define(function() {
    ...
    Page.on = function(elem, even, fn) {
        elem.attachEvent ? elem.attachEvent('on' + even,
        function() {
    ...
    Page.prototype.view = function() {
        ...
        if (conf.curr > 1 && conf.prev) {
            view.push('<a href="javascript:;" target="_self" class="layui-laypage-prev" data-page="' + (conf.curr - 1) + '">' + conf.prev + '</a>');
        }
        ...
        if (conf.pages > conf.groups && dict.end < conf.pages && conf.last && conf.groups !== 0) {
            view.push('<span>&#x2026;</span><a href="javascript:;" target="_self" class="layui-laypage-last" title="&#x5C3E;&#x9875;"  data-page="' + conf.pages + '">' + conf.last + '</a>');
        }
        ...
        if (conf.curr !== conf.pages && conf.next || dict.flow) {
            view.push((function() {
                return (dict.flow && conf.curr === conf.pages) ? '<span class="layui-laypage-nomore" title="&#x5DF2;&#x6CA1;&#x6709;&#x66F4;&#x591A;">' + conf.next + '</span>': '<a href="javascript:;" target="_self" class="layui-laypage-next" data-page="' + (conf.curr + 1) + '">' + conf.next + '</a>';
            } ()));
        }

        return '<div class="layui-box layui-laypage layui-laypage-' + (conf.skin ? (function(skin) {
            return /^#/.test(skin) ? 'molv': skin;
        } (conf.skin)) : 'default') + '" id="layui-laypage-' + that.config.item + '">' + view.join('') +
        function() {
            return conf.skip ? '<span class="layui-laypage-total">&#x5230;&#x7B2C; <input type="text" min="1" onkeyup="this.value=this.value.replace(/\\D/, \'\');" value="' + conf.curr + '" class="layui-laypage-skip"> &#x9875; ' + '<button type="button" target="_self" class="layui-laypage-btn">&#x786e;&#x5b9a;</button></span>': '';
        } () + '</div>';
    };
